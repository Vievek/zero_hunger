...
userSchema.methods.canAcceptDonation = async function (donationQuantity = 1) {
  ...
  try {
    ...
    console.error("Error in canAcceptDonation:", error);
    return false;
  }
};
...
userSchema.methods.getCurrentLoad = async function () {
  ...
  try {
    // Use mongoose.model() to get Donation model at runtime
    ...
    console.error("Error in getCurrentLoad:", error);
    return this.recipientDetails?.currentLoad || 0; // Fallback to stored value
  }
};
  ...
  userSchema.methods.getPerformanceMetrics = async function () {
    ...
    try {
      ...
      const reliabilityScore = Math.max(
        0,
        completionRate - cancellationPenalty
      );
  ...
  userSchema.virtual("recipientStats").get(function () {
    ...
    return {
      ...
      verificationStatus:
        this.recipientDetails?.verificationStatus || "pending",
