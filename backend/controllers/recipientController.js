...
exports.acceptDonationOffer = async (req, res) => {
  try {
    ...
    console.log(
      `✅ ${req.user.role} ${req.user.id} accepting donation: ${donationId}`
    );
    ...
    if (existingMatch) {
      ...
      if (recipientMatch.status === "offered") {
        ...
        console.log(
          `🔄 Updated existing match to accepted for donation: ${donationId}`
        );
      } else {
        console.log(
          `ℹ️ Recipient already has match with status: ${recipientMatch.status}`
        );
      }
    } else {
    ...
    const taskData = {
      ...
      scheduledPickupTime:
        donation.type === "bulk" && donation.scheduledPickup
    ...
    notificationService
      .sendStatusUpdate(
        ...
        "Donation Accepted! 🎉",
        `Your donation "${
          donation.aiDescription || donation.description || "Food Donation"
    ...
    console.log(
      `🎉 Donation ${donationId} successfully accepted by ${req.user.id}`
    );
    ...
    res.json({
      ...
      message:
        "Donation accepted successfully! A volunteer will be assigned for pickup soon.",
    });
  } catch (error) {
